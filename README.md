...
1. El comprador elige si el regalo **se le entrega a él mismo** (solo disponible en modo curada — llena su dirección ahí mismo y va directo por el checkout normal) o **a otra persona** (deja su nombre y WhatsApp para que le avisen).
2. Si es para otra persona, **el pago es obligatorio antes de generar el link** — no hay opción de "pagar después". En cuanto el comprador confirma el pago (Yape), se genera el link (`catalogo.html?regalo=<id>`) con un mensaje ya armado y personalizado (usa el nombre del comprador si lo dejó), que además es **editable** — el comprador puede agregarle algo antes de compartirlo por WhatsApp o copiarlo. El link es válido por **15 días** (`GIFT_EXPIRY_DAYS`, definido tanto en `assets/shop.js` como en el Worker — hay que mantenerlos iguales si se cambia).
3. La persona regalada abre el link, elige uno entre las opciones, llena su propia dirección de entrega y confirma.
4. El pedido **solo llega a la pestaña Pedidos una vez que la persona elige** — como el pago ya ocurrió antes de compartir, en la práctica el pedido se registra en cuanto la persona regalada confirma su elección.
5. Si nadie elige dentro de los 15 días, el link queda **expirado**: la próxima vez que alguien lo abre, el Worker lo detecta y le cambia el estado a `expirado` en KV (no hay Cron Trigger — es una revisión "perezosa" al momento de acceder, no un job en segundo plano). Un regalo ya convertido en pedido nunca expira.
